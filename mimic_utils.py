...
######################## Helpful Function ##########################################################
# Load from local(Drive) mimic dataset
# load when the file type is .csv.gz
...
# def cal_duration(df, start_col_name, end_col_name, new_col_name):
# 	"""
# 	calculate the different between two time stemp columns,
# 	adding new duration column in hour
# 	"""
# 	df[end_col_name] = pd.to_datetime(df[end_col_name])
# 	df[start_col_name] = pd.to_datetime(df[start_col_name])
# 	df[new_col_name] = (df[end_col_name]-df[start_col_name]).apply(lambda x: x.total_seconds() // 3600)
# 	return

def filter_col_value(df, name, bounds):
	"""
	bounds: should be a list includes [lower, upper]
	        lower and upper can be None, which represent no lower bound or upper bound
	return the df that drop the rows outside the bounds=[lower, upper]
	"""
	if bounds[0] == None:
		df = df[(df[name] <= bounds[1])]
	elif bounds[1] == None:
		df = df[(df[name] >= bounds[0])]
	else:
		df = df[(df[name] >= bounds[0]) & (df[name] <= bounds[1])]
	return df
####################### Demographics ##################################################################
# ------------------------------------------------------------------
